...
"""
    ...
    Notes: The language is set here because metro can be used as a library and metro_util is the module that
           every other module loads. Thus, the language setting that are used through metro is always set if
           this code is in the metro_util module.

    metro_logger cannot be invoked here since this module is imported in metro_logger.
...
def test_import(sModule_name):
    ...
    try:
        ...
        exec (sCode)
    except (SyntaxError, ImportError, EOFError) as inst:
...
def interpolate(xArray, yArray):
    """
        ...
        Parameters: [[I] Numeric.Array lXArray : The value of x to interpolate.  Must be of constant distance between
                    two consecutive X.]
                    [[I] Numeric.Array lYArray: The value of y to interpolate.]
                    [[I] int iIncrement: The increment to use between two X consecutive values to get the new X

        Returns: list The interpolate Y value. Throw an exception in case of error.

        Functions Called: [interp() ]

        Description: This function do the interpolation of a one dimension function.
                     The x and the corresponding y value are given(i.e. y[n] correspond to x[n]) The value of x must be
                     evenly spaced. The increment value is set in metro_constant by the value of fTimeStep and
                     tells how to increment the value between two consecutive x.

        Notes: This function needs the NumArray package. We assume that the values do no need to be sorted
               (i.e. they are in order).

        Revision History:

        Author		          Date		      Reason
        Miguel Tremblay      June 30th 2004   To replace the linear interpolation of fortran code
    """
    ...
    if iLenXArray != iLenYArray:
        ...
        sMetroUtilError = _("In interpolate, iIncrement is too big. \n") + _("Space between two values in xArray:") + \
                          "xArray[0]: %f xArray[1]:%f \n Increment=%f" % (xArray[0], xArray[1], iIncrement)
        raise metro_error.Metro_util_error(sMetroUtilError)
...
def shift_left(npInput, fValueAdded=0):
    """
        ...
        Parameters: [[I] numpy npInput : The array that the value will be shifted left]
        ...
        Description: This method shift the value of the array at left,
                     i.e. npInput[n] becomes npInput[n-1] for all n in [1..len(npInput)-1].
                     The value fValueAdded is added at the end, i.e. npInput[len(npInput)-1]=fValueAdded
    ...
    # Cut the first value
    npOutput = numpy.take(npInput, numpy.arange(1, len(npInput)))
...
def shift_right(npInput, fValueAdded=0):
    """
        ...
        Parameters: [[I] numpy npInput : The array that the value will be shifted right]
                    [[I] double fValueAdded=0 : The value to be added at the beginning of the array
        ...
        Description: This method shift the value of the array at right,
                     i.e. npInput[n] becomes npInput[n+1] for all n in [0..len(npInput)-2].
                     The value fValueAdded is added at the begining, i.e. npInput[0]=fValueAdded
    ...
    npOutput = numpy.take(npInput, numpy.arange(0, len(npInput) - 1))
...
def get_indice_of(npInput, fValue=0):
    """
        ...
        Description: The numpy must be ordered.
                    Returns the indices where npInput[nIndice-1] <= fValue <= npInput[nIndice]
    ...
    """
    if not isinstance(npInput, numpy.ndarray):
    ...
    for i in range(1, len(npInput)):
        if npInput[i - 1] <= fValue <= npInput[i]:
            return i
    sMetroUtilError = _("No index with this value: %d") % fValue
    raise metro_error.Metro_util_error(sMetroUtilError)
...
def get_difference_array(npInput, bPrevious=False):
    """
        ...
        Returns: numpy npOutput : An array storing the difference.
                 bool bPrevious : If True , get the difference between the index i and the index i-1.
                                  If False (default), get the difference between the index i and index i+1.
        Functions Called:

        Description: This method compute the difference between consecutive value in a numarrray.
...
def sign(dResult, dSign):
    """
        ...
        Notes: This is the equivalent of SIGN in fortran.
               And no, there is no built-in function in python that does that.
...
def subsample(npInput, nSubsamplingIndice):
    """
        ...
        Parameters: numpy npInput : array to subsample integer nSubsamplingIndice :
                    if == 2, only take one element out of two.
    ...
    if len(npInput) < nSubsamplingIndice:
        sMetroUtilError = _("In metro_util.subsample, subsampling rate") + \
                          _("is higher than array size: %d > %d") % (nSubsamplingIndice, len(npInput))
        raise metro_error.Metro_util_error(sMetroUtilError)
...
def concat_array(npArray1, npArray2):
    """
        ...
        Description: Take two arrays, transform then in column and then form a couple of number in each position.
                     Usefull to create graphics.

        x = [x1, x2, ..., xn]
        y = [y1, y2, ..., yn]
        concat_array(x,y) return [[x1,y1], [x2,y2], ..., [xn,yn]],
    ...
    nLen2 = len(npArray2)
    if nLen2 < nLen1:
        sMetroUtilWarning = _("Array are not of the same size") + _("cutting the first one")
        print(sMetroUtilWarning)
        ...
        sMetroUtilError = _("In metro_util.concat_array, array must be") + \
                          _(" of the same dimension: %d != %d") % (nLen1, nLen2)
        raise metro_error.Metro_util_error(sMetroUtilError)
...
def sum_array(npInput):
    ...
    for i in range(0, len(npInput)):
        npOutput = numpy.concatenate((npOutput, numpy.array([npInput[0:i].sum()])), 1)
    return npOutput
...
def validate_version_number(sVersion, sMin_version, sMax_version):
    ...
    if sVersion is not None:
        ...
        if version < min_version:
            sMessage = _("Version number:'%s' is too old. Version from '%s' ") % (version, min_version) + \
                       _("to '%s' inclusively are supported") % max_version
            raise metro_error.Metro_version_error(sMessage)
        elif version > max_version:
            sMessage = _("Version number:'%s' is not yet supported. Version ") % version + \
                       _("from '%s' to '%s' inclusively are supported") % (min_version, max_version)
            raise metro_error.Metro_version_error(sMessage)
    else:
        sMessage = _("Can't find version number. Version from '%s' ") % min_version + \
                   _("to '%s' inclusively are supported") % max_version
        raise metro_error.Metro_version_error(sMessage)
...
def interp(y_values, x_values, new_x_values):
    """
        ...
        Comes from http://projects.scipy.org/pipermail/scipy-user/2007-March/011479.html Thank you Stephen
  """
