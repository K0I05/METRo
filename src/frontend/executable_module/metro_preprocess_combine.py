...
"""
    ...
    Notes: TODO MT: Enlever le -3*3600/30 ou trouver une raison satisfaisante pour sa presence.
...
class Metro_preprocess_combine(Metro_preprocess):
    ...
    def start(self):
        ...
        self.__combine_AT(forecast_data.get_interpolated_data(), observation_data.get_interpolated_data(),
                          observation_data)
        self.__combine_TD(forecast_data.get_interpolated_data(), observation_data.get_interpolated_data(),
                          observation_data)
        self.__combine_WS(forecast_data.get_interpolated_data(), observation_data.get_interpolated_data(),
                          observation_data)
        self.__combine_QP(forecast_data.get_interpolated_data(), observation_data.get_interpolated_data())
    ...
    def __set_attribute(self, observation_data):
        """
            ...
            Description: Compute the number of 30 seconds step in the observation.
                         Uses self.observation_data.DELTA_T, i.e. the number of hours in the observation to do so.
        """
    ...
    def __combine_AT(self, wf_interpolated_data, ro_interpolated_data, observation_data):
        """
            Name: __combine_AT
        ...
        if len(npBadIndices) == 0:
            ...
            if fTaCorr != 0:
                if self.NTP2 < 0:
                    ...
                    nValueSup = nLenAT
                for i in range(nLenATO - self.NTP, int(nValueSup)):
                    npAT[i - self.NTP2] = npAT[i - self.NTP2] - math.exp(-(i - nFactor) * metro_constant.fTimeStep \
                                                                         * metro_constant.fConst) * fTaCorr
        wf_interpolated_data.set_matrix_col('AT', npAT)

    def __combine_TD(self, wf_interpolated_data, ro_interpolated_data, observation_data):
        """
            Name: __combine_TD
        ...
        if len(npBadIndices) == 0:
            ...
            if fTdCorr != 0:
                if self.NTP2 < 0:
                    ...
                    nValueSup = nLenTD
                for i in range(nLenTDO - self.NTP, int(nValueSup)):
                    npTD[i - self.NTP2] = npTD[i - self.NTP2] - math.exp(-(i - nFactor) * metro_constant.fTimeStep \
    ...
    def __combine_WS(self, wf_interpolated_data, ro_interpolated_data, observation_data):
        """
            Name: __combine_WS
        ...
        if len(npBadIndices) == 0:
            ...
            if fCurrentObs < fCurrentFor:
                ...
                fSlope = (1 - fTdCorr) * metro_constant.fConst * metro_constant.fTimeStep
                ...
                for i in range(int(round(nLenWSO - self.NTP)), nValueSup):
                    npWS[i - self.NTP2] = npWS[i - self.NTP2] - math.exp(-(i - nFactor) * metro_constant.fTimeStep \
                                                                         * metro_constant.fConst) * fTdCorr
        wf_interpolated_data.set_matrix_col('WS', npWS)

    def __combine_QP(self, wf_interpolated_data, ro_interpolated_data):
        """
            Name: __combine_QP
        ...
        for i in range(0, nLenPI - self.NTP):
            npQP[i - self.NTP2] = npPI[i + self.NTP - 1] * (npQP[i + 1 - self.NTP2])
