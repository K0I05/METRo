...
"""
    ...
    Note:         Observation file cannot be too long because of the limitation of length in the fortran array.
...
class Metro_preprocess_validate_input(Metro_preprocess):
    ...
    def start(self):
        ...
        self.__validate_last_observation_date(observation_data.get_controlled_data())
        self.__validate_overlap(forecast_data.get_controlled_data(), observation_data.get_controlled_data())

    def __validate_optional_args_forecast(self, forecast_data):
        """
            In the case the user asked to use infra-red and/or solar flux as input,
            check if the values are really in the input files.
        """
        # Check for solar-flux
        if metro_config.get_value('SF') and 'SF' not in forecast_data.lMatrix_col_name:
            sInputAtmosphericForecast = metro_config.get_value('FILE_FORECAST_IN_FILENAME')
        ...
        # Check for infra-red
        if metro_config.get_value('IR') and 'IR' not in forecast_data.lMatrix_col_name:
            sInputAtmosphericForecast = metro_config.get_value('FILE_FORECAST_IN_FILENAME')
    ...
    def __validate_forecast_length(self, forecast_data):
        ...
        for i in range(len(npFT)):
            nHourElapsed = metro_date.get_elapsed_time(npFT[i], npFT[0])
            if i != nHourElapsed:
    ...
    def __validate_observation_length(self, observation_data):
        ...
        if nNbr_30_seconds_step_in_obs > metro_constant.nNL:
            ...
            sMessage = _("Too many observation. Removing the %s seconds ") % nNumberSecondsToRemove \
                       + "i.e. %s hour(s)\n" % fNumberHourToRemove \
                       + "Old start time is %s\n" % sOldStartTime \
                       + "New start time is %s" % sNewStartTime
            metro_logger.print_message(metro_logger.LOGGER_MSG_WARNING, sMessage)
    ...
    def __validate_overlap(self, forecast_data, observation_data):
        """
            ...
            Description: Make sure forecast and observation overlap, if it's not the case abort METRo
        """
        ...
        if fForecast_start > fObservation_end:
            ...
            sError = _("Forecast and observation don't overlap. The date\n") + \
                     ...
                     _("observation.\nFirst Forecast='%s'\nLast Observation='%s'") % (sForecast_start, sObservation_end)
